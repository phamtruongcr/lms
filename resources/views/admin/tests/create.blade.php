@extends('layouts.app')
@section('title',('form/test.Create a test'))
...
<h2>{{__('form/test.Create a test') }}</h2>
<div>
  <form method="post" action="{{ route('test.store') }}">
  @include('admin/tests/_test_form')
  <button type="submit" class="btn btn-primary">{{__('form/test.Create') }}</button>
  </form>
</div>
@stop
        ...
        $.ajax({
          ...
          success: function(result){
            ...
            for(var i=0; i<result.list_question.length; i++){
              // total_point+=parseFloat(result.list_question[i][0].point);
